...
async def h_r(bot:Bot,event:PrivateMessageEvent,state: T_State):
    if '中午好' in str(event.get_message()):
        hour_num=datetime.now().hour
        if hour_num >=7 and hour_num <=10:
            msg = '宝贝，是不是睡眠不够脑袋不清楚了...现在是早晨！'
        elif hour_num >=10 and hour_num <= 14:
            msg = '中午好！'
        elif hour_num <=7:
            msg = '你是猫头鹰嘛！！我超喜欢猫头鹰！！主人也是...'
        elif hour_num >= 19:
            msg = '睡午觉睡昏了吧小猪崽！！！现在是晚上！'
        else:
            msg = '现在是下午茶时间，要来喝杯茶么?'
        msg = Message(msg)
...
async def h_r(bot:Bot,event:PrivateMessageEvent,state: T_State):
    if '晚上好' in str(event.get_message()):
        ...
        if hour_num >=7 and hour_num <=10:
            ...
            msg = '晚上好呀！今天也是收获满满的一天呢！！'
        else:
            msg = '明明还没有到晚上呢！让我再睡会儿！！'
        msg = Message(msg)
